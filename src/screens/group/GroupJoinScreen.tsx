...
export default function GroupJoinScreen() {
  ...
  const submitCode = useCallback(
    async (fullCode: string) => {
      ...
      try {
        ...
        // 호스트 본인이거나 이미 참여한 참가자가 같은 코드/링크로 재입장하는 경우,
        // 참여코드 재입력이나 프로필 재작성 없이 원래 그룹 화면으로 바로 보낸다.
        let existingMembership: GroupDetail | null = null;
        try {
          existingMembership = await getGroupDetail(String(result.groupId));
        } catch {
          // 403(아직 참여하지 않음) 등은 신규 참여자이므로 기존 참여 플로우로 진행한다.
        ...
        if (existingMembership) {
          router.replace(
            getGroupEntryRoute(
              String(existingMembership.groupId),
              existingMembership.myRole,
  ...
  // 초대 링크로 들어온 경우, 최초 1회 자동으로 코드 검증을 실행한다.
  useEffect(() => {
  ...
  return (
    ...
    >
      ...
      <main className={styles.main}>
        {isVerifyingLink ? (
          <div
            className={styles.linkVerifying}
            role="status"
            aria-live="polite"
          >
            <span className={styles.linkVerifyingSpinner} aria-hidden="true" />
            <p className={styles.linkVerifyingText}>
              초대 링크를 확인하고 있어요...
            </p>
          </div>
        ) : (
